...
FbxNode* CreateCube(FbxScene* pScene, const char* pName, FbxDouble3& pLclTranslation)
{
	...
	static Vector4 lControlPoints[8] = {
		{ -5.0,  0.0,  5.0, 1.0}, 
		{  5.0,  0.0,  5.0, 1.0}, 
		{  5.0,10.0,  5.0, 1.0},    { -5.0,10.0,  5.0, 1.0},
...
{	
	...
	// create the main structure.
	FbxMesh* lMesh = FbxMesh::Create(pScene, "dfefe");
	FbxNode* lNode = FbxNode::Create(pScene, pName);

	int pointcount = vertices.size();

	
	lMesh->InitControlPoints(pointcount);
...
vector<FbxVector4> CreatePoint(int p[], int len)
{
	...
	vector<FbxVector4> points;


	for (int i=0;i<len;i=i+3)
	{
		FbxVector4 pp = FbxVector4(*(p+i), *(p+i+1), *(p+i+2));
